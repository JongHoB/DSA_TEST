...
static int __init my_init(void)
{
	...
	entry = proc_create(PROC_VMALLOC_DST_NAME, 0, NULL, &my_proc_ops);
	if (!entry)
	{
		ret = -ENOMEM;
		goto out;
	...
	ret = register_chrdev_region(MKDEV(MY_MAJOR, 1), 1, "myvmap_dst");
	if (ret < 0)
	{
	...
	for (i = 0; i < NPAGES * PAGE_SIZE; i += PAGE_SIZE)
	{
		...
		sprintf(vmalloc_area + i, "vmalloc_dst %d", i / PAGE_SIZE);
	...
	pr_info("myvmap_dst module loaded\n");
...
out_no_chrdev:
	remove_proc_entry(PROC_VMALLOC_DST_NAME, NULL);
out:
	return ret;
}
...
static void __exit my_exit(void)
{
	...
	pr_info("myvmap_dst module unloaded\n");
}
