...
static void dsa_copy(void)
{
    ...
    list_for_each_entry(enabled_wq, &idxd_wqs, list)
    {
        ...
        int total_nents = min_t(int, src_sgts[enabled_wq->enabled_wq_num]->nents, dst_sgts[enabled_wq->enabled_wq_num]->nents);
        descs_entries[enabled_wq->enabled_wq_num] = total_nents;

        max_ents = max_t(int, max_ents, total_nents);
    }
